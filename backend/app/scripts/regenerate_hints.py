#!/usr/bin/env python3
"""
Regenerate all question hints with specific, helpful prompts.
Usage: python3 regenerate_hints.py [path_to_db]
"""
import sqlite3
import sys

DB_PATH = sys.argv[1] if len(sys.argv) > 1 else "/opt/interview-master/backend/data/questions.db"


def regenerate_hints():
    conn = sqlite3.connect(DB_PATH)
    conn.row_factory = sqlite3.Row
    rows = conn.execute("SELECT id, title, category, difficulty, hint FROM questions ORDER BY id").fetchall()

    updates = []

    for r in rows:
        new_hint = generate_tailored_hint(r["title"], r["category"], r["difficulty"])
        if new_hint != r["hint"]:
            updates.append((new_hint, r["id"]))

    for hint, qid in updates:
        conn.execute("UPDATE questions SET hint = ?, updated_at = datetime('now') WHERE id = ?", (hint, qid))

    conn.commit()
    conn.close()
    print(f"Updated {len(updates)} / {len(rows)} hints")

    # Show a few samples
    print("\n--- Sample changes ---")
    for hint, qid in updates[:5]:
        title = [r["title"] for r in rows if r["id"] == qid][0]
        print(f"\n[{qid}] {title[:60]}")
        print(f"  → {hint}")


def generate_tailored_hint(title: str, category: str, difficulty: str) -> str:
    t = title.lower()
    ...
    if category == "模型架构":
        ...
        if "structured output" in t or "结构化输出" in t:
            return "提示：从三个层次说明——①Prompt 约束（最弱，依赖模型遵循指令的意愿）；②JSON Mode（中等，保证 JSON 格式但不保证字段名和类型）；③Tool Calling / Structured Output API（最强，Schema 驱动，严格匹配）。每层给出适用场景和失败案例。"
